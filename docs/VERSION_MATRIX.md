...
| NVIDIA 驱动 | Ubuntu 官方驱动；当前合格主机可保留 `595.71.05`；验收版本不得低于 `560.35.05` | 最低版本加内容锁定 | Ubuntu 官方 NVIDIA Debian 包；驱动缺失或不合格时按 ADR-0004 单独评审事务，不在 Phase 1 自动安装 | `nvidia-smi --query-gpu=name,driver_version,memory.total --format=csv,noheader`；`dpkg --compare-versions "$(nvidia-smi --query-gpu=driver_version --format=csv,noheader \| head -n1)" ge 560.35.05` |
...
| ROS–Gazebo | `ros_gz 1.0.22-1` | 精确锁定 | ROS Jazzy Noble 软件源中的 `ros-jazzy-ros-gz` | `dpkg-query -W -f='${Package}\t${Version}\n' ros-jazzy-ros-gz \| grep -E $'^ros-jazzy-ros-gz\\t1\\.0\\.22-1([^0-9].*)?$'`；完整 Ubuntu 包 revision 还必须逐字匹配批准的环境清单 |
...
### 3.1 PyTorch CUDA wheel 内容身份工作流

`https://download.pytorch.org/whl/cu126` 只是候选索引，不是可重现 wheel 身份。Phase 1 在任何 hashed lock 写入前，必须为当前 Python 3.12/Linux x86_64 平台解析并下载唯一的 `torch==2.12.1` CUDA 12.6 wheel 与 `torchvision==0.27.1` CUDA 12.6 wheel，计算每个文件 SHA-256，并以 UTF-8 TSV 写入 `artifacts/environment/pytorch-cu126-wheels.tsv`。该文件按 logical name 排序，固定列为：`logical_name<TAB>resolved_url<TAB>filename<TAB>distribution_version<TAB>sha256`；两个 logical_name 只能是 `torch`、`torchvision`，URL 必须是 HTTPS 的 `download.pytorch.org/whl/cu126/` 完整 wheel URL，filename 必须与 URL basename 相同，版本必须分别精确解析为 `2.12.1+cu126` 与 `0.27.1+cu126`，摘要为 64 个小写十六进制字符。解析到零个或多个候选、非 Linux/Python 3.12 wheel、URL/filename/version 不一致或摘要未记录均为硬失败。

随后 `requirements.lock` 中这两个分发必须各自以记录的完整 `name @ resolved_url` 加 `--hash=sha256:<sha256>` 出现；不能只保留 `--extra-index-url`/`--index-url` 和版本号，也不能使用可变 HTML 索引、重定向最终地址、`latest` 或本地无摘要 wheel。安装前再次下载/读取的 wheel 必须同时匹配 TSV 的完整 URL、filename、版本和 SHA-256。`pip freeze` 只作为安装后证据，不能替代这份解析前内容身份。Phase 0 不执行解析、下载、建 venv 或生成 lock；该段规定未来脚本必须执行和归档的顺序。
...
外部资源的详细治理见 [DATA_AND_MODELS](DATA_AND_MODELS.md)。本表只定义版本身份和获取源；实际文件必须由 manifest 记录 SHA-256，且大文件不提交 Git。2026-07-23 起，公开训练数据下载和模型微调由用户在本仓库外完成；本仓库只保留数据来源身份、接收用户发布的不可变 GitHub 模型资产，并用官方 `yolo11n.pt` 作为开发占位/base weight。

| 资源 | 固定身份 | 获取源 | 锁定与验证 |
|---|---|---|---|
| 15-class Substation Equipment | revision `c63ed3c7f5ea33ac8e9024c467a70a62b849b2ad` | <https://huggingface.co/datasets/AndrzejDD/15-class-Substation-Equipment> | 外部用户训练参考；本仓库不下载。若用户训练 manifest 引用该来源，必须记录 revision、每文件 SHA-256、7,076 张总数和原始 split |
| Hard Hat Workers | Roboflow public dataset version `10` | <https://public.roboflow.com/object-detection/hard-hat-workers/10> | 外部用户训练参考；本仓库不下载。若用户训练 manifest 引用该来源，必须记录实际 YOLO 导出 URL、导出时间、archive SHA-256 和每文件 SHA-256 |
| D-Fire | commit `4bf9c31b18fadcd44d5f0b6d66f82bc56fa5e328` | <https://github.com/gaia-solutions-on-demand/DFireDataset> | 外部用户训练参考；本仓库不下载。若用户训练 manifest 引用该来源，Git checkout 必须处于该 commit，文件清单摘要进入 dataset manifest |
| InsPLAD | 使用时解析并固定一个完整 40 位 commit；许可固定为 CC BY-NC 3.0 | <https://github.com/andreluizbvs/InsPLAD> | 外部用户训练参考；本仓库不下载。若用户训练 manifest 引用该来源，未写入完整 commit 和文件 SHA-256 前不得进入验收 |
...
| YOLO11n COCO 基础权重 | Ultralytics YOLO11n，文件名 `yolo11n.pt`，release `v8.4.0` | `https://github.com/ultralytics/assets/releases/download/v8.4.0/yolo11n.pt` | Phase 1 官方占位/base weight；实际 SHA-256 记录在 `artifacts/environment/resource-downloads.tsv`，生产别名不得指向该占位权重 |
| 项目模型 | `yolo11n_safety.pt`、`yolo11n_equipment.pt`、`yolo11n_fault.pt`、独立仪表定位权重 `yolo11n_meter.pt`，以及其下游 OpenCV 仪表读数配置/产物 | 用户后续发布的不可变 GitHub release 或固定 commit 资产 | 四个模型 artifact 分别记录内容 SHA-256、训练 Git commit 或 release tag、数据 manifest SHA-256、指标和用途；`yolo11n_meter.pt` 只定位表盘，OpenCV 独立完成透视/刻度/指针读数；生产别名只能指向 manifest 中已通过验收的 artifact |
