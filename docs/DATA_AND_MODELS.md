...
本文规定公开数据、Gazebo 合成数据、标注转换、训练、评估、模型发布和追溯规则。通常仓库只保存 manifest、脚本、许可/引用说明和校验值；原始公开数据、转换数据、日志与 rosbag2 不直接提交 Git。Phase 4 本轮按用户明确要求保留其上传的四模型训练 ZIP `artifacts/phase4/substation_yolo_runs.zip`，作为不可变 handoff 证据，不把 ZIP 内的训练日志或原始数据拆入仓库。

2026-07-23 起执行职责固定如下：公开训练数据下载和模型微调由用户在本仓库外完成，本仓库不执行公开数据下载、标注转换或训练，也不搜索或接纳来源不明的第三方生产模型。官方 `yolo11n.pt` 仅作为开发占位/base weight，用于环境、接口和推理管线联调，不得进入生产 artifact 映射。严格发布仍要求用户不可变 GitHub release 或固定 commit；本轮用户明确授权的本地 ZIP handoff 通过 `scripts/import_phase4_models.py` 接入，并在 manifest 中显式记录来源类型与 safety mAP 阈值 waiver。项目仍负责 Gazebo 合成场景和数据生成能力，并把其 manifest 作为可交给用户训练的输入证据。
...
- 本仓库不创建公开数据的 `raw` 或 `derived` 内容；若用户训练 manifest 引用这些路径，它们只描述仓库外训练事实。只有项目自有 Gazebo 生成器可在本服务器创建 `synthetic` 内容。
- 路径身份只用于定位，内容身份始终是 SHA-256。不得覆盖已有 revision、conversion、generation 或 model artifact。
- 用户发布包中的权重、评估报告、数据 manifest、训练配置摘要和校验值必须保留到关联运行/报告的证据保留期结束。
- 用户训练产物必须记录训练代码 Git commit 或不可变 release 身份；dirty working tree、浮动分支或缺少来源身份的产物不得晋升为项目模型。

## 3. 数据源登记

| `dataset_id` | 用途 | 固定来源与 revision | 数量基线 | 许可与允许用途 | 接纳规则 |
|---|---|---|---:|---|---|
| `substation-equipment-15` | 设备检测主数据 | [15-class Substation Equipment](https://huggingface.co/datasets/AndrzejDD/15-class-Substation-Equipment)，revision `c63ed3c7f5ea33ac8e9024c467a70a62b849b2ad` | train 5,023；val 601；test 1,452；总计 7,076 | Apache-2.0；按许可用于本项目训练、评估和可复现研究 | 外部用户训练参考；仓库不下载。用户训练 manifest 必须保持原始 split，并记录数量、revision、YOLO 标注和全部文件 SHA-256 |
| `hard-hat-workers-v10` | 人员/PPE 安全检测 | [Hard Hat Workers v10](https://public.roboflow.com/object-detection/hard-hat-workers/10)，版本 10 | 7,041 张 | CC0；可用于安全检测训练/评估 | 外部用户训练参考；仓库不下载。用户训练 manifest 必须记录 YOLO 导出 URL、导出时间、archive 和逐文件 SHA-256 |
| `d-fire` | 火焰/烟雾安全检测和负样本 | [D-Fire](https://github.com/gaia-solutions-on-demand/DFireDataset)，commit `4bf9c31b18fadcd44d5f0b6d66f82bc56fa5e328` | 超过 21,000 张；fire 框 14,692；smoke 框 11,865；无烟火负样本 9,838 | CC0 1.0；可用于安全检测训练/评估 | 外部用户训练参考；仓库不下载。用户训练 manifest 必须固定该 commit、记录内容清单 SHA-256 并保留负样本 |
| `insplad` | 设备检测补充与独立缺陷分类 | [InsPLAD](https://github.com/andreluizbvs/InsPLAD)：10,607 张 Full-HD 图、28,933 个实例、17 类资产；`InsPLAD-fault` 含 5 类资产、6 种状态 | 以接纳 manifest 的精确统计为准 | **CC BY-NC 3.0**；仅限非商业教学研究，派生模型和演示必须保留署名、许可与非商业限制 | 外部用户训练参考；仓库不下载。用户训练 manifest 必须固定完整 40 位 commit、archive 和逐文件 SHA-256，不得跟随默认分支 |
...
本表是训练建议和接纳契约，不是仓库下载队列。不得直接重新发布来源数据。任何来源的许可文件、引用文本、主页 URL、revision 和用途限制都必须随用户训练 manifest 保存；许可未知、冲突或无法验证时拒绝对应模型资产。
...
该 manifest 由用户训练发布包提供，仓库导入后作为模型来源证据保存；仓库不会根据它下载公开数据。manifest 使用 UTF-8 YAML，顶层 key 固定并按字典序序列化后计算文件 SHA-256。schema 1 至少包含：
...
每个 `file_manifest_sha256` 必须与发布包内 `file_manifest_path` 指向的同一份按相对路径字节序排序的 TSV 内容摘要清单相匹配，路径以发布包 manifest 根为基准、使用 `/`、不得为绝对路径、不得含 `..`、空段或反斜杠。TSV 行格式为 `sha256<TAB>size_bytes<TAB>relative_path`。仓库校验器必须重算这份清单自身的 SHA-256；原始公开数据不随模型发布包传输，也不由本仓库重新下载。绝对路径、用户名和下载 token 不得进入受控 manifest。
...
模型微调由用户执行；本节是用户产物进入项目时必须证明的训练基线，而不是本仓库的训练任务。安全检测、设备检测、缺陷分类和独立仪表定位四个 YOLO11n artifact 共用以下不可变起点：
...
用户发布包至少包含四个不可变权重、`models/manifest.yaml`、`datasets/manifest.yaml`、数据文件清单、训练配置摘要和 metrics 文件；GitHub release tag 或固定 commit、每个资产 URL、字节数和 SHA-256 必须能够互相核对。缺少任一项时，本仓库继续使用官方占位权重做非生产联调，并将生产模型集成标为 blocked。
...
artifacts:
  - logical_model: yolo11n_safety
    module: safety
    source_repository: user-controlled-github-repository
    source_revision: immutable-release-tag-or-40-char-commit
    source_asset_url: immutable-github-asset-url
...
示例只定义结构。`source_repository` 必须是用户控制的 GitHub 仓库，`source_revision` 必须是不可变 release tag 或完整 40 位 commit，`source_asset_url` 必须指向该固定身份下的资产；浮动 branch、latest URL 或第三方重新打包资产均被拒绝。实际 manifest 中 `acceptance_status` 只允许 `passed` 或 `rejected`；不存在 `pending` 发布状态。尚未完成评估的训练产物不得出现在 production 映射。四个 `required_logical_models` 都必须各有一个独立、已通过的 production SHA-256；`meter_locator` 还必须精确映射到 `yolo11n_meter.pt`，并引用独立 OpenCV 配置摘要。任一缺失都使 manifest 校验失败，且不能改变“仪表只使用 Gazebo 数据”的限制。
...
该脚本在创建前不是 Phase 0 可运行命令。它只验证已经导入的用户发布包和项目自有 Gazebo synthetic manifest，不触发数据下载或训练。实现必须以非零退出码拒绝以下任一情况：schema/必填字段不符、GitHub 来源不是固定 release/commit、source `status` 非 accepted、blocked/rejected 的 `block_reason` 语义不符、file_manifest_path 缺失/越界/不存在/摘要不匹配、许可或 revision 缺失、SHA-256/字节数不匹配、跨 split 重复证据缺失、未知/未映射类别、仪表外部数据、合成样本不足、训练环境不可追溯、指标缺失、阈值未达、生产映射指向未通过 artifact 或文件名/摘要不一致。
