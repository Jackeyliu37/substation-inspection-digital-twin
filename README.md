...
Phase 0 文档与契约已完成并固定；Phase 1“环境基线”fast-track 正在执行。documentation gate、acceptance run、轻量主机预检以及 Node.js 24.18.0 和官方 `yolo11n.pt` 占位/base weight 下载已完成；尚未安装系统依赖、创建虚拟环境、构建前端或启动 Gazebo/Nav2/Web 服务。公开训练数据下载和模型微调由用户在仓库外完成，后续生产权重只从用户发布的不可变 GitHub release 或固定 commit 拉取并验收。当前事实与下一命令以 [`PROJECT_STATUS`](docs/PROJECT_STATUS.md) 和 [`HANDOFF`](docs/HANDOFF.md) 为准。
