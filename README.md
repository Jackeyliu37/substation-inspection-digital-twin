...
datasets/          用户训练数据 manifest 与 Gazebo 合成数据说明
...
Phase 0～3 已完成并有环境、Gazebo、导航 immutable evidence；Phase 4 官方模型占位 smoke 已完成，四个生产模型仍等待用户从 AutoDL 发布。Phase 5～8 已有受限开发实现：风险驱动任务重排的 Gazebo live acceptance、Gateway REST/WebSocket 契约、八页前端及生产构建均已通过。真实 Gateway ROS 适配、报告 Service、完整 Nav2 巡检执行、Playwright/Windows/Nginx 联调和最终演示仍未完成，不能宣称已部署产品服务。当前事实、验证结果和恢复命令以 [`PROJECT_STATUS`](docs/PROJECT_STATUS.md) 与 [`HANDOFF`](docs/HANDOFF.md) 为准。
