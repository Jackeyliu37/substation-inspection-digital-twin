...
- 本轮全量软件验证：`colcon build --symlink-install && colcon test && colcon test-result --all --verbose` 为 `174 tests, 0 errors, 0 failures, 0 skipped`；Gateway、部署及 Phase 5～6 顶层 contract 为 `28 passed`，documentation gate 通过，world/navigation/perception/synthetic 的既有检查点仍有效。
