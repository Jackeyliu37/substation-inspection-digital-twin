...
function Dashboard({ robot, mission, assets, alerts, events, onCommand, onRecover, disabled }) {
  ...
  return <><div className="metric-grid"><Metric label="机器人模式" value={robotModeLabel(robot?.mode)} detail={robot?.current_task_id ? `任务 ${robot.current_task_id.slice(0, 8)}` : "机器人在线"} /><Metric label="电量" value={robot?.battery_percent != null ? `${robot.battery_percent.toFixed(0)}%` : "--"} detail={robot?.stale ? "位姿已过期" : "实时仿真电量"} /><Metric label="高风险资产" value={highRisk} detail={`${assets.length} 个孪生资产`} tone="warning" /><Metric label="风险事件" value={alerts.length} detail="当前运行事件流" tone="danger" /></div><div className="dashboard-grid"><section className="panel mission-panel"><PanelTitle title="巡检任务" action={mission?.route_id ? "默认巡检路线" : "等待路线"} /><div className="progress"><i style={{ width: `${Math.round((mission?.progress_0_1 ?? 0) * 100)}%` }} /></div><div className="mission-stats"><strong>{missionStateLabel(mission?.state ?? "idle")}</strong><span>{mission?.completed_tasks ?? 0} / {mission?.total_tasks ?? 0} 台 · {Math.round((mission?.progress_0_1 ?? 0) * 100)}% 完成</span></div><div className="operator-guide"><strong>人工验收</strong><span>机器人到达一台设备后会立即完成该设备评估并前往下一台。请观察完成数量、资产风险和事件流逐台更新。</span></div><div className="button-row"><button className="primary-action" onClick={onRecover} disabled={disabled || autonomousRunning}>{autonomousRunning ? "自动巡检运行中" : "恢复并开始自动巡检"}</button><button className="secondary" onClick={() => onCommand("/api/v1/missions/pause", missionBody)} disabled={disabled || mission?.state !== "running"}>暂停</button><button className="secondary" onClick={() => onCommand("/api/v1/missions/resume", missionBody)} disabled={disabled || mission?.state !== "paused"}>继续</button><button className="secondary" onClick={() => onCommand("/api/v1/missions/stop", missionBody)} disabled={disabled || !["ready", "running", "paused", "stopping"].includes(mission?.state)}>停止</button></div></section><section className="panel asset-overview"><PanelTitle title="资产状态" action={`${assets.length} 个设备`} />{assets.length ? <AssetRows assets={assets} /> : <EmptyState title="等待资产数据">数字孪生状态尚未到达。</EmptyState>}</section><section className="panel event-panel"><PanelTitle title="事件流" action="最近 50 条" />{events.length ? events.slice(-50).reverse().map((event, index) => <div className="event" key={`${event.type}-${index}`}><span className="event-dot" /><div><strong>{event.payload?.asset_id ? assetLabel(event.payload.asset_id) : eventLabel(event.payload?.kind ?? event.type)}</strong><span>{event.payload?.description ?? eventLabel(event.type)}</span><small>{event.payload?.occurred_at ?? event.timestamp ?? "实时事件"}</small></div></div>) : <EmptyState title="运行平稳">尚无新的风险或命令事件。</EmptyState>}</section></div></>;
}
...
function PerceptionView({ cameraUrl, cameraMeta, cameraFps, models, robot, assets, mission }) {
  const activeTask = mission?.tasks?.find((task) => task.task_id === mission.active_task_id);
  const activeAsset = assets.find((asset) => asset.asset_id === activeTask?.asset_id) ?? assets.find((asset) => asset.risk?.level !== "unknown") ?? null;
  const measurement = activeAsset?.measurements ?? {};
  const risk = activeAsset?.risk ?? {};
  const value = (number, unit = "") => number == null ? "暂无读数" : `${Number(number).toFixed(2)}${unit}`;
  return <div className="perception-layout"><section className="panel camera-panel"><PanelTitle title="实时感知画面" action={cameraUrl ? `${cameraFps.toFixed(1)} 帧/秒 · 机器人相机` : "等待相机"} />{cameraUrl ? <><div className="camera-portrait"><img className="camera-frame camera-frame-rotated" src={cameraUrl} alt="左转九十度后的机器人实时标注相机画面" /></div><div className="camera-meta"><span>{cameraMeta?.captured_at ?? "实时机器人相机帧"}</span><span>{cameraMeta?.annotated ? "已叠加检测结果" : "原始画面"}</span></div></> : <EmptyState title="等待第一帧">数据服务已连接，正在等待机器人相机画面。</EmptyState>}</section><aside className="sensor-column"><section className="panel sensor-panel"><PanelTitle title="机器人实时传感器" action={robot?.stale ? "数据延迟" : "实时更新"} /><dl className="sensor-grid"><dt>当前检查设备</dt><dd>{activeAsset ? assetLabel(activeAsset.asset_id) : "等待任务"}</dd><dt>里程计位姿</dt><dd>{robot?.pose ? `${robot.pose.x_m.toFixed(2)}, ${robot.pose.y_m.toFixed(2)} 米` : "暂无读数"}</dd><dt>线速度 / 角速度</dt><dd>{robot?.twist ? `${robot.twist.linear_x_m_s.toFixed(2)} 米/秒 · ${robot.twist.angular_z_rad_s.toFixed(2)} 弧度/秒` : "暂无读数"}</dd><dt>电池电量</dt><dd>{value(robot?.battery_percent, "%")}</dd><dt>相机帧率</dt><dd>{cameraUrl ? `${cameraFps.toFixed(1)} 帧/秒` : "暂无画面"}</dd><dt>温度传感器</dt><dd>{value(measurement.temperature_celsius, " °C")}</dd><dt>烟雾传感器</dt><dd>{value(measurement.smoke_0_1)}</dd><dt>气体传感器</dt><dd>{value(measurement.gas_ppm, " ppm")}</dd><dt>仪表读数</dt><dd>{value(measurement.meter_reading, measurement.meter_unit ? ` ${measurement.meter_unit}` : "")}</dd></dl></section><section className="panel risk-formula"><PanelTitle title="风险评分方法" action={activeAsset ? `${assetLabel(activeAsset.asset_id)} · ${value(risk.score_0_100, " 分")}` : "等待评估"} /><div className="formula">总分 = 100 × 加权分量之和</div><ul><li><span>视觉异常 × 30%</span><b>{value(risk.visual_0_1)}</b></li><li><span>温度 × 25%</span><b>{value(risk.temperature_0_1)}</b></li><li><span>烟雾 × 20%</span><b>{value(risk.smoke_0_1)}</b></li><li><span>气体 × 10%</span><b>{value(risk.gas_0_1)}</b></li><li><span>组合场景 × 15%</span><b>{value(risk.context_0_1)}</b></li></ul><div className="formula-result">当前等级：{riskLabel(risk.level)}</div></section></aside><section className="panel model-panel model-status-wide"><PanelTitle title="模型状态" action="生产权重已接入" />{models.length ? <div className="model-list">{models.map((model) => <div className={`model-card ${model.installed ? "installed" : "missing"}`} key={model.logical_model}><div><strong>{modelLabel(model.logical_model)}</strong><span>{model.installed ? "运行权重已安装" : "权重缺失"}</span></div><small>{model.logical_model}</small><small>验证指标：{model.best_metric != null ? Number(model.best_metric).toFixed(3) : "--"}</small><small>{model.classes?.length ?? 0} 个识别类别 · 校验码 {model.sha256?.slice(0, 10)}…</small></div>)}</div> : <EmptyState title="模型清单不可用">正在读取生产模型清单。</EmptyState>}</section><ModelShowcase /></div>;
}
