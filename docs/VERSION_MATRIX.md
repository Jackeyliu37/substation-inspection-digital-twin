...
| npm | 随 Node.js `24.18.0` 分发的版本，以 `package-lock.json` 的 `packageManager` 字段精确记录 | Node.js 官方发行包 | `npm --version` 与 `node -p 'require("./web/frontend/package.json").packageManager'` 一致 |
| Next.js | `16.2.11` | npm registry；<https://nextjs.org/docs> | `web/frontend/package-lock.json`；`npm ls next --depth=0` |
| React / React DOM | `19.2.8` / `19.2.8` | npm registry；<https://react.dev/> | `package-lock.json`；`npm ls react react-dom --depth=0` |
| TypeScript | `6.0.3` | npm registry；<https://www.typescriptlang.org/docs/> | `package-lock.json`；`npm ls typescript --depth=0` |
| Tailwind CSS | `4.3.3` | npm registry；<https://tailwindcss.com/docs> | `package-lock.json`；`npm ls tailwindcss --depth=0` |
| Three.js | `0.185.1` | npm registry；<https://threejs.org/docs/> | `package-lock.json`；`npm ls three --depth=0` |
| React Three Fiber | `9.6.1` | npm registry；<https://r3f.docs.pmnd.rs/> | `package-lock.json`；`npm ls @react-three/fiber --depth=0` |
| Apache ECharts | `6.1.0` | npm registry；<https://echarts.apache.org/en/index.html> | `package-lock.json`；`npm ls echarts --depth=0` |
| Playwright | `@playwright/test@1.61.1` | npm registry；<https://playwright.dev/docs/intro> | `package-lock.json`；`npx playwright --version \| grep -Fx 'Version 1.61.1'` |

前端只允许在 Ubuntu 服务器执行 `npm ci` 和生产构建。`package.json` 中上述直接依赖必须写精确版本，`package-lock.json` 使用 lockfile v3 并提交 Git；Windows 操作员端不安装 Node.js 或依赖。
...
| 项目模型 | `yolo11n_safety.pt`、`yolo11n_equipment.pt`、`yolo11n_fault.pt`，以及仪表定位/读数模块产物 | 仅由已批准 manifest 数据和固定训练配置生成 | 每个不可变 artifact 记录内容 SHA-256、训练 Git commit、数据 manifest SHA-256、指标和用途；生产别名只能指向 manifest 中已通过验收的 artifact |
