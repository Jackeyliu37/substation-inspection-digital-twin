...
阶段 0“文档门槛”已完成，阶段 1“环境基线”是当前活动阶段和下一执行阶段。阶段 1 必须严格从 [`docs/plans/PHASE-01-ENVIRONMENT.md`](docs/plans/PHASE-01-ENVIRONMENT.md) 的 Task 1 开始；计划中尚未创建的脚本、测试和服务命令目前仍是验收合同，只有对应文件按计划创建后才成为可运行入口。当前尚未执行 Phase 1：未安装依赖、未下载数据或模型、未启动 Gazebo/Nav2/Web 服务，也未创建 ROS 2 功能包。
