...
- 开发检出下真实相机能发布 640×480 `rgb8`，修复后的生产节点均能输出；正式 15 FPS/300 秒结果尚未宣称通过，必须在 root 安装 release 后用新 run ID 执行。
- 浏览器验收按操作员决定为人工验收，不使用 Playwright；不要求截图或演示视频交付。
...
- Phase 9 部署检查点：systemd、Nginx、Foxglove 和 safe-stop 契约已重新验证；Gateway/接口/部署回归为 `49 passed`，documentation gate 为 `PASS`。Nginx 片段通过临时 `events/http` 包装配置的 `nginx -t` 语法检查；immutable release 构建与五服务依赖已完成，产品服务未启动，root 安装、Windows LAN、真实 Foxglove 和人工 Web 验收仍待现场执行。
...
- Phase 6 速度仲裁检查点：`4268803` + production integration closure。任务模块现在是最终 `/cmd_vel` 单一项目发布者：autonomous 只接受 `/cmd_vel_nav`，manual 只接受带 run/context 的 `/cmd_vel_manual`；手动命令使用进程 monotonic deadline，校验模式、紧停、frame、限速和 duration，按 ACCEPTED→APPLIED 发布状态并到期归零，重复 command 不重放。`/mission/set_robot_mode` 使用 state/latch revision CAS，IDLE→START 和紧停复位 0.5 s/无活动 goal motion barrier 已实现并测试。
...
- Phase 7 Gateway 机器人状态检查点：`e0578d0` + production perception closure。Gateway 使用显式契约 QoS 订阅 `/odom`、`/battery_state` 和 tf2，在 odom 样本的精确 stamp 查询 `map←odom`，验证 frame/有限数/四元数后组成 map pose，并把标准 0～1 电量换算成百分数。mission 模式、锁存 revision 和 active task 与机器人快照原子同步；pose 超过 0.5 秒或时钟异常即 stale，HTTP gate 与 ROS 发布边界均拒绝普通手动移动，紧停路径不受影响。真实相机 JPEG 与四模块生产 Topic 已接入并有聚焦测试。
