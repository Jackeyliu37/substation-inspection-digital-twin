...
“速率”是仿真运行时名义发布率；`事件`表示状态变化时立即发布且不轮询，`事件 + N Hz` 表示变化立即发布并以 N Hz 重发当前状态。所有节点名是逻辑所有者，实际可组合进同一进程但不能改变所有权。表内简写固定映射为：数字孪生=`substation_digital_twin`，风险=`substation_risk`，任务=`substation_mission`，报告=`substation_reporting`，Gateway=`substation_web_gateway`，证据采集/存储=架构中证据存储拥有的采集节点，检测聚合器=`substation_perception/detection_aggregator`。除 `/clock`、`/system/run_context` 和纯 TF/诊断基础设施外，表内每个 ROS 发布者都必须先订阅 `/system/run_context`；只有 ACTIVE 上下文才可产生带 run 归属的样本或证据。
...
| 运行上下文 | `/system/run_context` | `substation_mission/task_manager`（唯一发布者） | 所有项目 ROS 发布者、Gateway、报告和证据存储 | `substation_interfaces/msg/RunContext` | 状态变化立即发布；`Q_STATE` 为后加入者保留最新值 | `Q_STATE` |
...
任务管理器是 operational run 的唯一创建者和状态机所有者。它先把新上下文与 mission 初始快照原子持久化，再发布 `RunContext`；任何其他节点只能消费。合法生命周期为 `IDLE -> STARTING -> ACTIVE -> ENDING -> ENDED -> IDLE`，每次转换严格增加持久化的 `context_revision`，任务管理器重启后也不得倒退。

- `IDLE` 明确表示当前没有 run：`run_id=""`，started/ended time 为零，reason_code=`NO_ACTIVE_RUN`。空字符串只在该状态合法，不是 UUID 占位符。
- `STARTING` 首次携带新 UUID run_id；只允许任务管理器准备 route、mission、审计和执行器，其他节点不得产生 run-scoped 派生结果或证据。
- `ACTIVE` 允许发布者产生带 run 归属的派生状态和证据。所有显式 `run_id` 字段必须逐字等于最新 RunContext；不匹配样本拒绝并报 `RUN_CONTEXT_MISMATCH`。
- `ENDING` 禁止新任务和新的传感/检测证据采集，只允许取消导航、归零、完成既有证据写入，并由报告/清单生产者生成终态证据完成封口；`ENDED` 表示这些 barrier 已完成并保留结束 run_id 供消费者提交最终状态。
- 从 `ENDED` 转为 `IDLE` 后，基础设施 `/clock`、TF、`/diagnostics` 和用于 readiness 的原始传感器仍可发布，但不得分配 evidence_id 或被计入某个 run。其余 run-scoped publisher 抑制输出。
...
任何不符合表中 frame 的样本都以 `FRAME_ID_INVALID` 拒绝，不能由 Gateway 根据 Topic 名猜 frame。`/cmd_vel_nav`、`/cmd_vel` 是标准 `Twist`，没有 header；它们的线/角速度按 `base_link` 约定解释。手动速度的 frame 与时间由 `ManualVelocityCommand.header` 明示。
...
`header.frame_id="base_link"`，stamp 是 Gateway 发布命令时的 ROS time。`duration_s` 范围 `[0.05,0.25]`；command_id 是对应 REST 命令 UUID。任务安全仲裁器按 command_id 去重，拒绝未来时间超过 0.02 s、收到时已超过 stamp+duration 或不匹配当前手动模式/RunContext 的消息。
...
必须带 Idempotency-Key；只在 `mode=manual`、ACTIVE RunContext、非紧停、robot state 非 stale 时接受。`linear_x_m_s` 范围 `[-0.4,0.4]`，`angular_z_rad_s` 范围 `[-0.8,0.8]`，`duration_s` 为有限数且范围 `[0.05,0.25]`，非零速度要求 `deadman=true`。每个浏览器会话最高 10 请求/s；命令 1 s 超时。Gateway 发布携带同一 command_id 的 `ManualVelocityCommand` 到 `/cmd_vel_manual`，不得发布裸 `Twist`；任务安全仲裁器回发 `/mission/manual_velocity_status` 后，Gateway 才按第 9.4 节终结 REST 命令。duration 到期、松键、浏览器失焦、WebSocket 心跳丢失或进程断线时仲裁器立即选择零速度，并对尚未 APPLIED 的请求发布 EXPIRED/CANCELLED。成功受理为 202；失败码：409 `MANUAL_MODE_REQUIRED`、`EMERGENCY_STOP_LATCHED`、`RUN_CONTEXT_MISMATCH`；422 `VELOCITY_LIMIT_EXCEEDED`、`DEADMAN_REQUIRED`；429 `MANUAL_COMMAND_RATE_EXCEEDED`；503 `ROBOT_STATE_UNAVAILABLE`。
...
- 手动速度的 HTTP command succeeded 只表示收到 matching `ManualVelocityStatus.APPLIED`：该 twist 已被安全仲裁器选中并至少一次发布为最终 `/cmd_vel`，不保证物理位移已发生。duration 到期不创建新 command，而是安全仲裁器自动归零。
...
| `robot.manual_velocity` | `/mission/manual_velocity_status` 出现 matching `command_id=C` 的 terminal state=APPLIED；ACCEPTED 不终结命令。 | `applied_at`、`duration_s` |
...
| 命令终态 | `MANUAL_COMMAND_EXPIRED` | 手动样本在首次 APPLIED 前超过 stamp+duration。 |
