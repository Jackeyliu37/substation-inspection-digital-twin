...
Proceed to the next Phase 1 checkpoint: planned dependency/toolchain setup, lock capture, empty ROS workspace baseline, AI/Gateway virtual environments, frontend baseline, and headless EGL smoke path. Preserve Ubuntu 24.04, ROS 2 Jazzy, Gazebo Harmonic OGRE2/EGL headless rendering and the FastAPI-only product browser boundary.
