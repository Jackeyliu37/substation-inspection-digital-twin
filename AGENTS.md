...
10. [PROJECT_STATUS](./docs/PROJECT_STATUS.md) 和 [HANDOFF](./docs/HANDOFF.md) 存在后，每个阶段性检查点验证结束都必须同步更新它们，记录固定 commit、逐字验证命令、结果、阻塞项和下一步；不得把全部状态更新推迟到阶段最后。用户明确授权的个人项目 fast-track 可以把原阶段计划中的多个细粒度 task 合并为较少检查点，但不能跳过硬性环境边界、哈希证据、运行身份和完成证据。
11. 公开训练数据下载和模型微调由用户在本仓库外完成；本仓库不下载公开训练数据，也不搜索第三方生产模型。官方 `yolo11n.pt` 仅作开发占位/base weight，生产模型只接收用户发布的不可变 GitHub release 或固定 commit 资产并校验 manifest、SHA-256、指标和允许用途。
12. 不修改原始公开数据；转换数据、训练产物、日志和 rosbag2 不直接提交 Git，仓库只保存 manifest、脚本和校验值。
13. 发现规范冲突时停止相关实现，明确指出冲突并先更新权威文档；不得自行选择一种解释继续开发。
