...
所有项目服务、源码、数据、模型、SQLite 与运行证据集中在 Ubuntu 服务器。开发检出可位于授权操作员账户；不可变 release 位于 `/opt/substation/releases/<git-commit>`，`/opt/substation/current` 原子指向已验证 release，systemd 统一以运行账户 `substation` 执行。可变状态位于 `/var/lib/substation`，日志位于 `/var/log/substation`。`ROS_LOCALHOST_ONLY=1`；FastAPI 与 Next.js 分别只绑定 `127.0.0.1:8000` 和 `127.0.0.1:3000`，Nginx 才能向局域网公开 TCP/80 的 `http://ros-server/`、`/api/` 与 `/ws/`。Gateway 是产品 Web UI 浏览器到 ROS 的唯一状态和命令边界；SQLite 不提供网络数据库服务。Foxglove Bridge 只绑定 `127.0.0.1:8765`、默认关闭，维护时经 Nginx `/foxglove/` 代理；客户端发布、Service、Action 和参数写能力全部关闭。

## 后果

- 普通操作员只需浏览器；Windows 无需安装项目依赖，所有可追溯证据保留在服务器。
- Nginx、Gateway、前端和 ROS 形成可独立检查的进程边界，Gateway 负责命令校验、限频和断线降级。
- 单操作员受信 LAN 本期继续使用 HTTP，不增加应用认证或 TLS；部署必须禁止公网路由。任何公网、多操作员或跨不可信网络暴露必须先由新的 ADR 定义认证、TLS、授权和审计。
