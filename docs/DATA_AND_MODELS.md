...
2026-07-23 起执行职责固定如下：公开训练数据下载和模型微调由用户在本仓库外完成，本仓库不执行公开数据下载、标注转换或训练，也不搜索或接纳来源不明的第三方生产模型。官方 `yolo11n.pt` 仅作为开发占位/base weight，用于环境、接口和推理管线联调，不得进入生产 artifact 映射。用户完成训练后，生产模型必须来自用户发布的不可变 GitHub release 或固定 commit；本仓库只负责拉取、校验、集成和验收该发布资产。项目仍负责 Gazebo 合成场景和数据生成能力，并把其 manifest 作为可交给用户训练的输入证据。
