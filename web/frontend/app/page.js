...
export default function HomePage() {
  ...
  const renderView = useMemo(() => ({
    ...
    map: <MapView robot={robot} onCommand={sendCommand} disabled={controlsDisabled} />,
    ...
    reports: <ReportsView connection={connection} />,
    system: <SystemView system={system} events={events} onRefresh={refresh} />,
  }), [alertItems, assetItems, cameraUrl, connection, controlsDisabled, events, mission, refresh, robot, sendCommand]);
...
function ReportsView({ connection }) { return <section className="panel reports-panel"><PanelTitle title="巡检报告" action="证据不可变" /><EmptyState title={connection === "live" ? "等待报告列表" : "Gateway 不可用"}>{connection === "live" ? "报告将从 `/api/v1/reports` 读取并保留下载摘要。" : "恢复连接后加载报告与诊断包。"}</EmptyState></section>; }
