...
- 本轮全量软件验证：`colcon build --symlink-install && colcon test && colcon test-result --all --verbose` 为 `174 tests, 0 errors, 0 failures, 0 skipped`；Gateway、部署及 Phase 5～6 顶层 contract 为 `37 passed`，documentation gate 通过，world/navigation/perception/synthetic 的既有检查点仍有效。
...
| Phase 7 Gateway 真实控制面 | 权威状态、mission/evidence/reporting、mode/manual/e-stop、精确 TF/odom pose 与电量已接入 | 补控制命令终态、report/diagnostic 索引下载与真实相机帧 |
