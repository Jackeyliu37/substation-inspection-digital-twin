...
运行环境是 Ubuntu 24.04 服务器。ROS 2 Jazzy、Gazebo Harmonic、GPU 推理、SQLite、FastAPI、Next.js 和 Nginx 都在服务器上运行。普通操作员只使用 `http://ros-server/`；浏览器从不连接 ROS DDS，也不发布 ROS Topic。Foxglove 仅是开发诊断旁路，不是日常操作路径。
...
| `substation_gazebo` | Harmonic SDF 世界、OGRE2/EGL 无头物理与传感器仿真、可重复场景事件和仿真真值 | 使用描述与配置，发布原始 RGB、LiDAR、温度、烟雾/气体和仿真状态；真值仅供场景控制、验收和证据，绝不进入感知推理 |
...
| `substation_web_gateway` | ROS `rclpy` 适配、状态聚合/限频、单位转换、JPEG 编码、REST/WebSocket、命令校验和 `command_id` 生命周期 | 消费 ROS 状态并调用任务/导航安全接口和证据存储；只监听本机回环地址，唯一的浏览器—ROS 边界 |
...
| Foxglove Bridge / Foxglove Web | 开发者经认证诊断 ROS Topic、TF、地图和图像 | 只读诊断旁路；不能成为普通操作员控制路径，也不替代 Gateway、前端或证据记录 |
...
| 诊断 | Foxglove Bridge 与浏览器 Foxglove Web | 仅开发/排错时启用；与日常 Web 入口分离 |

## 4. 主数据流与控制流

```text
Gazebo Harmonic (OGRE2/EGL, headless)
  ├─ RGB ────────────────> 感知：安全 / 设备 / 缺陷 / 仪表
  ├─ 温度、烟雾/气体 ────> 归一化感知 ─┐
  ├─ LiDAR ──────────────> SLAM / Nav2 │
  └─ 场景真值 ───────────> 场景验收与证据（不进推理）
...
Gateway 以 REST 提供快照和命令受理，以 WebSocket 提供实时遥测、事件和 JPEG 图像。浏览器控制请求首先经过 Gateway 的验证、模式检查和命令生命周期；任务管理器才可创建或修改 Nav2 任务。紧急停止保留 Gateway 到 ROS 安全控制接口的优先路径：取消导航、发送零速度、锁存停止，且必须显式复位后才可恢复。
