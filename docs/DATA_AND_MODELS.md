...
仪表模块使用 YOLO11n 定位表盘，OpenCV 完成透视校正、圆盘/刻度/指针分析。输出不是自由文本类别，而是 `reading`、资产配置提供的 `unit`、`confidence_0_1`、`valid` 和 `evidence_id`，经 `/perception/meters/readings` 发布。
...
| 仪表读数 | 仅 Gazebo meter 冻结 test 场景 | 定位 AP、有效读数率、绝对误差分布、按表型/光照/视角/模糊/遮挡分组结果、推理速度 | 根项目计划没有授权数值下限；外部仪表数据、真值泄漏、单位不来自配置、分组结果缺失或样本少于 2,000 即失败 |
...
其中 `logical_model` 只能为 `yolo11n_safety`、`yolo11n_equipment` 或 `yolo11n_fault`；`training_run_id` 是 UUIDv4；`first16` 是文件完整 SHA-256 的前 16 个小写十六进制字符。逻辑部署名 `yolo11n_safety.pt`、`yolo11n_equipment.pt`、`yolo11n_fault.pt` 只能通过 manifest 的 `production_artifact_sha256` 指向一个已通过门槛的不可变文件，不能覆盖旧文件。
...
示例只定义结构。实际 manifest 中 `acceptance_status` 只允许 `passed` 或 `rejected`；不存在 `pending` 发布状态。尚未完成评估的训练产物留在 training 目录且不得出现在 production 映射。仪表模块若产生定位权重，也按相同 immutable artifact 规则登记，但它不能改变“仅 Gazebo 数据”的限制。
