...
- Phase 4 占位运行时实现提交：`456e824e21c7fff8ede1b89675b95be843793498`；run ID `6781c716-b692-415e-9be7-cb4b355bc0b6`，结果 `passed`，immutable evidence `/var/lib/substation/evidence/acceptance/6781c716-b692-415e-9be7-cb4b355bc0b6/04-perception-placeholder`。该结果明确为 `development_placeholder`、`production_ready=false`，不是 Phase 4 生产验收。
- 当前运行的项目服务：无。
...
- Phase 4 占位 smoke 已执行：`bash tests/perception/run_placeholder_smoke.sh --expected-commit 456e824e21c7fff8ede1b89675b95be843793498`；probe 为 `backend_ready=true`、`inference_device=cuda:0`，相机/检测/带框图像计数为 75/13/12，13/12 个输出 header 分别匹配源帧，13 帧成功、60 帧被最新帧背压替换、0 帧失败。
- Phase 4 占位证据只读复核：`(cd /var/lib/substation/evidence/acceptance/6781c716-b692-415e-9be7-cb4b355bc0b6/04-perception-placeholder && sha256sum -c SHA256SUMS)`。官方权重 SHA-256 为 `0ebbc80d4a7680d14987a577cd21342b65ecfd94632bd9a8da63ae6417644ee1`、大小 `5613764` 字节；验收后无残留项目进程。
